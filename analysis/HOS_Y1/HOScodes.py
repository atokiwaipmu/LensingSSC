...
from analysis.HOS_Y1.aperture_mass_computer import measureMap3FromKappa
from analysis.HOS_Y1.Peaks_minima import find_extrema
from analysis.HOS_Y1.DSS_functions import DSS_class
...
class kappacodes(kappamaps):
    ...
    def run_map2alm(self,Nmap1,Nmap2=None,is_cross=False):
        if is_cross:
            ...
            map2_dir = self.dir_results+'map2/'
            os.makedirs(map2_dir,exist_ok=True)
            fn_out = map2_dir + fn_header.split('.')[0]+'_Nmap%d_%d_map2_Cell_ell_0_5000.dat'%(Nmap1+1,Nmap2+1)

...
class KappaAnalyser:
    ...
    def _analyse_patch(self):
        for patch_kappa_path in self.patch_kappa_paths:
            ...
            for patch_snr_path in tmp_snr_paths:
                fname = os.path.basename(patch_snr_path).replace('snr_patches', 'analysis_sqclpdpm')
                output_path = os.path.join(self.flat_dir, fname)
    ...
    def _check_preprocessed_data(self):
        ...
        self.patch_snr_paths = sorted(glob.glob(os.path.join(self.patch_dir, 'snr*.npy')))
        self.patch_kappa_paths = sorted(glob.glob(os.path.join(self.patch_dir, 'kappa*.npy')))

    def _filter_paths_by_input(self, paths, input_path):
        info = InfoExtractor.extract_info_from_path(input_path)
        noise = os.path.basename(input_path).split('_')[-1].split('.')[0]
        return self._filter_paths(paths, info, noise)
        
    def _filter_paths(self, paths, info, noise):

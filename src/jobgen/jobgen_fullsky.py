...
def main():
    ...
    # Generate job script
    relative_path = "src.pipeline_fullsky"
    for config_id in ['tiled', 'bigbox']:
        script_dir = os.path.join(job_dir, "scripts", f"fullsky_{config_id}")
        ...
        for z in config_data.zs_list:
            for sl in config_analysis.sl_arcmin:
                for survey in ['noiseless', 'Euclid-LSST', 'DES-KiDS', 'HSC', 'Roman']:
                    jobname = f"fullsky_{config_id}_{z:.1f}_{sl}_{survey}"
    ...
    generator.generate_submission_script("fullsky")

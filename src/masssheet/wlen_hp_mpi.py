...
def make_massmap_dask(client, cat, npix):
    ...
    # Calculate the ipix values as a Dask array
    ipix = pid % npix
    ipix = ipix.astype('i4')
    
    # Using bincount directly on Dask arrays
    # We need to ensure that the length of the output matches npix
    mass_map_for_slice = da.bincount(ipix, weights=mass, minlength=npix)
...
def make_kappabar(Om, ds_list, zmin, zmax, bins=10000):
    dmin = Planck15.comoving_distance(zmin)
    dmax = Planck15.comoving_distance(zmax)

    drange = np.linspace(dmin, dmax, bins)
    zrange = Planck15.redshift(drange)
    step = drange[1] - drange[0]

    kappabar_list = []
    for ds in ds_list:
        LensKernel_list = np.array([wlen(Om, d, z, ds) for d, z in zip(drange, zrange)])
        kappabar = np.trapz(LensKernel_list, dx=step) / (dmax - dmin)

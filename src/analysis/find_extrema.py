...
def find_extrema_worker_xor(i: int, pixel_val: np.ndarray, neighbour_vals: np.ndarray, minima: bool = False) -> np.ndarray:
    if minima:
        return np.all(np.tile(pixel_val, (8, 1)).T < neighbour_vals, axis=-1)
    else:
        return np.all(np.tile(pixel_val, (8, 1)).T > neighbour_vals, axis=-1)
    
def find_extrema_xor(kappa_map: np.ma.MaskedArray, 
                minima: bool = False, 
                lonlat: bool = False,
                nside: int = None
    ...
    try:
        with mp.Pool(processes=mp.cpu_count()) as pool:
            extrema_chunks = pool.starmap(
                find_extrema_worker_xor, 
                [(i, pixel_val_chunks[i], neighbours_chunks[i], minima) for i in range(len(pixel_val_chunks))]
            )
    except Exception as e:
        print(f"An error occurred during multiprocessing: {e}")
        return np.array([]), np.array([])

    extrema = np.concatenate(extrema_chunks)

    if minima:
        print(f'Number of minima identified: {np.sum(extrema)}')
    else:
        print(f'Number of peaks identified: {np.sum(extrema)}')

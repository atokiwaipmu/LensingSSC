...
def find_extrema(kappa_map: np.ndarray,
                 lonlat: bool = False, nside: int = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    neighbour_vals = kappa_map[neighbours.T]
    pixel_val = kappa_map[ipix]
    ...
    try:
        with mp.Pool(processes=mp.cpu_count()) as pool:
            results = [pool.apply_async(find_extrema_worker, args=(i, pixel_val_chunks[i], neighbours_chunks[i])) for i in range(len(pixel_val_chunks))]
            extrema_chunks = []
            for result in tqdm(results, desc="Processing", total=len(results)):

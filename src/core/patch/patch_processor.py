...
class PatchProcessor:
    ...
    def __init__(self, patch_size_deg: float = 10.0, xsize: int = 2048) -> None:
        """
        ...
        points_path = f"/lustre/work/akira.tokiwa/Projects/LensingSSC/src/core/fibonacci/center_points/fibonacci_points_{patch_size_deg}.txt"
        if Path(points_path).exists():
            self.center_points = np.loadtxt(points_path)
            self.npatch = self.center_points.shape[0]
        else:
            raise FileNotFoundError(f"Center points file not found at {points_path}")
    ...
    def _get_valid_points(self) -> np.ndarray:
        ...
        valid_mask = (self.center_points[:, 0] < (np.pi - angular_radius_rad)) & (self.center_points[:, 0] > angular_radius_rad)

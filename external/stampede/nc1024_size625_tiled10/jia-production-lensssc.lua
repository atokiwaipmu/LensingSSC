...
n_steps = 60    --? first use 10 as a trial to test i/o
ai = 0.1        --?
af = 1
...
--lc_usmesh_tiles = fastpm.outerproduct({-4, -3, -2, -1, 0, 1, 2, 3}, {-4, -3, -2, -1, 0, 1, 2, 3}, {-4, -3, -2, -1, 0, 1, 2, 3})
lc_usmesh_healpix_nside = 8192
lc_usmesh_nslices = 100
lc_usmesh_fof_padding = 10.0   --? roughly the size of a halo.
lc_usmesh_alloc_factor = 16.   --? allocation factor for the unstructured mesh, relative to alloc_factor. increasing it?
...
loc = "/scratch/09665/atokiwa/LSSC/nc1024_size625_tiled10"
filename = string.format("/proc%d_nc%d_size%d_nsteps%d_s%d_10tiled", os.get_nprocs(), nc, boxsize, n_steps, random_seed)  --add time_step to


-- filename = string.format("/rfof_proc%d_nc%d_size%d_nsteps%dlin_ldr%d_rcv%s_fst%s_pnf%d_lnf%d_s%d_dhf%.4f_tiled%.2f_fll_elllim_%d_npix_%d_rfofkdt_%d_%s_10tiled_legacy", os.get_nprocs(), nc, boxsize, n_steps, linear_density_redshift, remove_cosmic_variance, force_softening_type, pm_nc_factor, lpt_nc_factor, random_seed, dh_factor, fof_linkinglength, lc_usmesh_ell_limit,lc_usmesh_healpix_nside,rfof_kdtree_thresh,growth_mode)  --add time_step to
